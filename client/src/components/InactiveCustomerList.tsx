...
import { 
  FaStore, FaUser, FaPhone, FaCalendar, FaCheckSquare, FaRegSquare, 
  FaSearch, FaPaperPlane, FaCheck, FaSort, FaChevronDown, FaChevronRight,
  FaShoppingBag, FaCalendarDay, FaReceipt, FaPills
} from "react-icons/fa";
...
const fetchCustomerPurchaseHistory = async (customerId: number) => {
  const route = API_ROUTES.CUSTOMER_PURCHASE_HISTORY.replace(":customerId", customerId.toString());
  const { data } = await api.get(route);
  return data;
};

// Purchase history component for better code organization
const CustomerPurchaseHistory = ({ purchaseHistory }: { purchaseHistory: any }) => {
  ...
  return (
    <div className="mt-2 bg-blue-50 rounded-lg p-2 text-sm">
      {Object.entries(purchaseHistory).map(([month, data]: [string, any]) => (
        <div key={month} className="mb-2 border border-blue-200 rounded-lg overflow-hidden">
          <button
          ...
          >
            ...
            <div className="flex items-center">
              <span className="mr-4 text-blue-100">Bills: {data.totalBills} | ₹{data.totalAmount}</span>
            ...
            {expandedMonth === month && (
              ...
              >
                <div className="bg-white px-2">
                  {Object.entries(data.dailyData).map(([day, dayData]: [string, any]) => (
                    <div key={day} className="my-2 border border-blue-100 rounded-lg overflow-hidden">
                      <button
                      ...
                      >
                        ...
                        <div className="flex items-center">
                          ...
                          {expandedDate === day ? <FaChevronDown /> : <FaChevronRight />}
                        </div>
                      </button>
                          ...
                          >
                            <div className="bg-white p-1">
                              {dayData.bills.map((bill: any) => (
                                <div key={bill.billNo} className="my-1 border border-blue-50 rounded-lg overflow-hidden">
                                  <button
                                  ...
                                  >
                                    ...
                                    <div className="flex items-center">
                                      <span className="mr-4 font-medium">₹{bill.amount}</span>
                                      {expandedBill === bill.billNo ? <FaChevronDown className="text-blue-500" /> : <FaChevronRight className="text-blue-500" />}
                                    </div>
                                  </button>
                                    ...
                                    {expandedBill === bill.billNo && (
                                      ...
                                      >
                                        <div className="p-2 bg-white">
                                          <div className="text-gray-500 mb-1 ml-2 text-xs">Medicines purchased:</div>
                                          <div className="overflow-x-auto">
                                            <table className="min-w-full divide-y divide-gray-100 text-xs">
                                              <thead>
                                                <tr className="bg-gray-50">
                                                  <th className="px-2 py-1 text-left text-gray-500">Medicine</th>
                                                  <th className="px-2 py-1 text-right text-gray-500">Qty</th>
                                                </tr>
                                              </thead>
                                              <tbody>
...
const InactiveCustomerList: React.FC<InactiveCustomerListProps> = ({ onCopyToTelecalling, fromDate, toDate }) => {
  ...
  const handleSelectCustomer = useCallback((customer: InactiveCustomer, event: React.MouseEvent) => {
  ...
  const handleCustomerClick = useCallback(async (customer: InactiveCustomer) => {
  ...
  const handleCopyToTelecalling = useCallback(() => {
    onCopyToTelecalling(selectedCustomers);
    setSelectedCustomers([]);
    setSelectAll(false);
  }, [selectedCustomers, onCopyToTelecalling]);
    ...
    >
      ...
      <div>
        ...
        <div className="bg-white p-2 border-b border-blue-100 flex items-center justify-between sticky top-0 z-10">
          <div 
            className="flex items-center cursor-pointer px-3 py-1 bg-blue-100 rounded-md hover:bg-blue-200"
            onClick={handleSelectAll}
          >
            {selectAll ? (
              <FaCheckSquare className="text-blue-700 text-lg mr-2" />
            ) : (
              <FaRegSquare className="text-blue-500 text-lg mr-2" />
            )}
                ...
                sortedData.map((customer, index) => (
                  <React.Fragment key={customer.id}>
                    <motion.tr 
                      ...
                      className={`${selectedCustomers.some(c => c.id === customer.id) ? "bg-blue-50" : ""} ${expandedCustomerId === customer.id ? "bg-blue-100" : "hover:bg-blue-50"} cursor-pointer`}
                      onClick={() => handleCustomerClick(customer)}
                    >
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <div 
                            className="cursor-pointer flex items-center justify-center w-6 h-6 rounded-md hover:bg-blue-200"
                    ...
                    <AnimatePresence>
                      {expandedCustomerId === customer.id && (
                        <motion.tr
                          ...
                          transition={{ duration: 0.2 }}
                        >
                          <td colSpan={6} className="px-6 py-2 bg-blue-50 border-t border-b border-blue-100">
                            <div className="py-2">
      ...
      {selectedCustomers.length > 0 && (
        <motion.div 
          className="sticky bottom-0 bg-white shadow-lg border-t border-blue-200 p-4 flex justify-between items-center"
        ...
        >
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="flex items-center px-6 py-2 bg-blue-700 text-white rounded-md hover:bg-blue-800 shadow-md"
            onClick={handleCopyToTelecalling}
          >
            <FaPaperPlane className="mr-2" />
            Copy to Telecalling
