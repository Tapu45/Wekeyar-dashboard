...
const fetchStoreWiseSalesReport = async (date: string): Promise<StoreWiseSalesReport> => {
...
const StoreWiseSalesReportPage: React.FC = () => {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["storeWiseSalesReport", selectedDate],
    queryFn: () => fetchStoreWiseSalesReport(selectedDate),
    enabled: !!selectedDate,
  });

  // Filter stores based on search query
  const filteredStores = data?.storeReports.filter((store: StoreReport) => 
  ...
  return (
    ...
    >
      ...
      <div className="p-6">
        <div className="flex flex-col md:flex-row gap-4 md:items-center mb-6 justify-between">
          ...
          >
            <div className="flex items-center gap-3 bg-blue-50 p-3 rounded-lg">
              ...
              <p className="text-blue-800 font-medium">
                Report for {data.selectedDate} - {searchQuery ? `${filteredStores.length} of ${data.storeReports.length} stores` : `${data.storeReports.length} stores`}
              </p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {(searchQuery ? filteredStores : data.storeReports).map((store: StoreReport, index: number) => (
                <motion.div
                  key={index}
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    {[
                        ...
                        const trendLabel = key.replace("previous", "Previous ");
                        return (
                          <div
                            key={key}
            ...
            {searchQuery && filteredStores.length === 0 && (
              <div className="p-10 text-center bg-blue-50 rounded-xl">
                ...
                <p className="text-xl font-medium text-blue-800">No stores found matching "{searchQuery}"</p>
                <p className="mt-2 text-blue-600">Try a different search term</p>
              </div>
            )}
          </motion.div>
        ) : (
